...
if(isset($_POST["id"]) && !empty($_POST["id"])){
    ...
    if(empty($nome_err)){
        // Query de Update
        $sql = "UPDATE diretor SET nome_dir=? WHERE cod_dir=?";
         
        if($stmt = mysqli_prepare($link, $sql)){
            // Setando as variáveis no prepareStatement
            mysqli_stmt_bind_param($stmt, "ss", $param_nome,$param_cod_dir);
    ...
    if(isset($_GET["id"]) && !empty(trim($_GET["id"]))){
        ...
        // Query de Select
        $sql = "SELECT * FROM diretor WHERE cod_dir = ?";
        if($stmt = mysqli_prepare($link, $sql)){
            // Setando as variáveis no prepareStatement
            mysqli_stmt_bind_param($stmt, "s", $param_cod_dir);
            ...
            <div class="col-md-12 d-flex align-items-stretch grid-margin">
              <div class="row flex-grow">
                <div class="col-12">
                  <div class="card">
                    <div class="card-body">
                      <h4 class="card-title">Editar Diretor</h4>
                      <form class="forms-sample" action="<?php echo htmlspecialchars(basename($_SERVER['REQUEST_URI'])); ?>" method="post">
                        <div class="form-group" <?php echo (!empty($nome_err)) ? 'has-error' : ''; ?>>
                          ...
                          <input type="text" class="form-control" id="nomeDiretor" name="nome" placeholder="Informe o nome do Diretor" value="<?php echo $nome; ?>">
